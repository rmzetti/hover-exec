...
export function activate(context: vscode.ExtensionContext) {
  ...
  function setExecParams(context:vscode.ExtensionContext,doc: vscode.TextDocument,
    pos: vscode.Position,line: vscode.TextLine){
      ...
      cmd='';cmda='';cmdId='';comment='';full='';mpe='';//reset start line parameters
  ...
  function getSpecialPath(s:string){
    //special path/name in command line or first line comment
      ...
      if(v.includes('/')){
        ...
        if(tempPath.startsWith('/')){tempFsPath=tempFsPath.slice(1);}
      } else {
        tempName=v.slice(v.lastIndexOf('\\')+1);
  ...
  function parseLine(s:string){
    function posComment(s:string){ //find start of comment in command line
        let ipos=s.indexOf('<!--');//using these comment indicators
        if(ipos<=0){ipos=s.replace('://','xxx').indexOf('//');}
        if(ipos<=0){ipos=s.indexOf('#');}
        return ipos;
    }
    inline=!s.toLowerCase().includes('noinline');//allows normal use of =>>
    if(!inline){s=s.replace(/noInline|noinline/,'');}
    ...
    if(/^\w/.test(s)){
      cmda=s.replace(/^(\w*).*/,'$1');
      if(/^\w+\s?:\w/.test(s)){cmdId=s.replace(/^\w*\s?:(\w*).*/,'$1');}
      else {cmdId=cmda;}
    }
  ...
  async handleUri(uri: vscode.Uri): Promise<void | null | undefined> {
    ...
    vscode.window.withProgress({  //set up status bar exec indicator
      ...
      title: 'Hover-exec'     //title for progress display
        ...
        if(showOk){progress1('ok',500);} //report successful completion
        };
        // hStatusBarItem.hide();
    });
