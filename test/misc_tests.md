...
## Contents
- [Hover-exec comparative tests](#hover-exec-comparative-tests)
- [Contents](#contents)
...
>      `js` or `js:vm` to use vscode's virtual machine
>      `js:node` to use node
>      `js:eval` to use vscode's eval
>      `html` to use javascript in the browser
...
These tests all use the same data utilising *hover-exec*'s *include* macro. 
To see the data quickly, hover over a line containing an *include* tag reference (not in preview).
...
<script>
let a=[ `include  #p1` ]   //note *include* is inside the array designator square brackets
